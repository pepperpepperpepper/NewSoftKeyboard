...
OUT_DIR="${1:-outputs/devicefarm/nextword-smoke/$(date +%Y%m%d-%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
FILTER="${DEVICEFARM_TEST_FILTER:-wtf.uhoh.newsoftkeyboard.app.dictionaries.presage.NextWordSuggestionsUiAutomatorTest#smoke}"
...
: "${DEVICEFARM_JOB_TIMEOUT_MINUTES:=25}"
