...
OUT_DIR="outputs/genymotion/nextword-smoke/$(date +%Y%m%d-%H%M%S)"
mkdir -p "${OUT_DIR}"
...
if [[ -z "${TEST_BUILD_TYPE:-}" ]]; then
  echo "TEST_BUILD_TYPE not set; defaulting to '${TEST_BUILD_TYPE_VALUE}' for Genymotion next-word smoke."
fi
...
TEST_CLASS="wtf.uhoh.newsoftkeyboard.app.dictionaries.presage.NextWordSuggestionsUiAutomatorTest"

echo "Running next-word suggestions smoke..."
echo "NOTE: this test may download the DistilGPT-2 sanity model on first run (network required)."
set +e
