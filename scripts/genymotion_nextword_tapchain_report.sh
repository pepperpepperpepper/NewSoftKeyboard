...
OUT_DIR="outputs/genymotion/nextword-tapchain-report/$(date +%Y%m%d-%H%M%S)"
mkdir -p "${OUT_DIR}"
...
TEST_CLASS="wtf.uhoh.newsoftkeyboard.app.dictionaries.nextword.NextWordTapChainReportUiAutomatorTest"

echo "Running next-word tap-chain report..."
INSTRUMENT_ARGS=()
...
if grep -q "^NWJSONGZ:" "${REPORT_LINES}"; then
  if ! command -v base64 >/dev/null 2>&1; then
    echo "Missing base64 in PATH; cannot decode NWJSONGZ report." >&2
    exit 1
  fi
  if ! command -v gzip >/dev/null 2>&1; then
    echo "Missing gzip in PATH; cannot decode NWJSONGZ report." >&2
    exit 1
  fi
...
UPLOAD_KEY="nsk-nextword-suite/index.html"
if [[ -n "${ONLY_CASE_ID:-}" ]]; then
  SAFE_ONLY_CASE_ID="${ONLY_CASE_ID//[^A-Za-z0-9._-]/_}"
  UPLOAD_KEY="nsk-nextword-suite-only-${SAFE_ONLY_CASE_ID}/index.html"
fi
URL="$(
