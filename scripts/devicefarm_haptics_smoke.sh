...
OUT_DIR="${1:-outputs/devicefarm/haptics-smoke/$(date +%Y%m%d-%H%M%S)}"
mkdir -p "${OUT_DIR}"
...
FILTER="${DEVICEFARM_TEST_FILTER:-wtf.uhoh.newsoftkeyboard.app.ime.DeviceFarmHapticsDiagnosticsTest#smoke}"
...
LOG_JSON="$(find "${OUT_DIR}" -type f -path "*/LOG/ListArtifactType.log_*" -print -quit || true)"
if [[ -n "${LOG_JSON}" && -f "${LOG_JSON}" ]] && command -v jq >/dev/null 2>&1; then
  jq -r '.[] | select(.tag=="DeviceFarmHaptics") | "\(.timestamp) \(.level) \(.data)"' "${LOG_JSON}" \
    > "${OUT_DIR}/devicefarm_haptics_diagnostics.txt" || true
  echo "Extracted DeviceFarmHaptics logs to: ${OUT_DIR}/devicefarm_haptics_diagnostics.txt"
fi
