...
public class KeyboardViewBaseTest {
  OnKeyboardActionListener mMockKeyboardListener;
  KeyboardDefinition mEnglishKeyboard;
  private KeyboardViewBase mUnderTest;
  ...
  public void setUp() throws Exception {
    ...
    KeyboardViewBase view = createViewToTest(getApplicationContext());
  ...
  protected KeyboardViewBase createViewToTest(Context context) {
    return new KeyboardViewBase(context, null);
  }
  ...
  public void testLongPressOutput() {
    KeyboardKey key = (KeyboardKey) mEnglishKeyboard.getKeys().get(5);
    key.longPressCode = 'z';
  ...
  public void testLongPressCallback() {
    KeyboardKey key = (KeyboardKey) mEnglishKeyboard.getKeys().get(15);
    key.longPressCode = 'z';
  ...
  public void testLongPressOutputTagsToast() {
    KeyboardKey key = Mockito.mock(KeyboardKey.class);
  ...
  public void testWithLongPressOutputLongPressKeyPressState() {
    final KeyboardKey key = findKey('f');
    key.longPressCode = 'z';
    KeyDrawableStateProvider provider =
        new KeyDrawableStateProvider(
            R.attr.key_type_function,
  ...
  public void testDefaultAutoCase() {
    final KeyboardKey fKey = findKey('f');
  ...
  public void testFunctionPreviewUsesFunctionLabels() {
    ...
    KeyboardDefinition numericKeyboard =
        NskApplicationBase.getKeyboardFactory(getApplicationContext())
            .getEnabledAddOn()
            .createKeyboard(Keyboard.KEYBOARD_ROW_MODE_NORMAL);
    ...
    for (int i = 0; i < allKeys.size(); i++) {
      ...
      if (candidate.getPrimaryCode() >= '0' && candidate.getPrimaryCode() <= '9') {
        digitKey = (KeyboardKey) candidate;
        digitIndex = i;
        break;
      }
    }
  ...
  @NonNull
  protected KeyboardKey requireFindKey(int codeToFind) {
    KeyboardKey key = findKey(codeToFind);
    Assert.assertNotNull(key);
    return key;
  }

  @Nullable
  protected KeyboardKey findKey(int codeToFind) {
    final int index = findKeyIndex(codeToFind);
    if (index == -1) {
      return null;
    } else {
      return (KeyboardKey) mUnderTest.getKeyboard().getKeys().get(index);
    }
  }
