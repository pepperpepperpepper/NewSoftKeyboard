...
public class ImeServicePhysicalKeyboardTest extends ImeServiceBaseTest {
  ...
  public void testStatusBarIconLifeCycle() {
    ...
    Assert.assertTrue(getShadowInputMethodManager().isStatusIconShown());
    if (mImeServiceUnderTest.onShowInputRequested(0, false)) {
  ...
  public void testKeyboardViewHiddenWhenPhysicalKeyPressed() {
    ...
    long time = 0;
    mImeServiceUnderTest.onKeyDown('c', new TestKeyEvent(time, KeyEvent.ACTION_DOWN, 'c'));
    mImeServiceUnderTest.onKeyUp('c', new TestKeyEvent(time, KeyEvent.ACTION_UP, 'c'));
  ...
  public void testKeyboardReOpenOnInputConfigurationChange() {
    ...
    if (mImeServiceUnderTest.onShowInputRequested(0, true /*configChange*/)) {
      mImeServiceUnderTest.onStartInputView(editorInfo, true);
    }
    // this is the same input field, but it was previously finished completely.
    Assert.assertFalse(mImeServiceUnderTest.isKeyboardViewHidden());
