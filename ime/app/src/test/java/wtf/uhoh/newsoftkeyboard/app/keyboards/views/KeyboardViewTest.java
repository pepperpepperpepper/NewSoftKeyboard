...
import static wtf.uhoh.newsoftkeyboard.app.keyboards.Keyboard.EDGE_LEFT;
import static wtf.uhoh.newsoftkeyboard.app.keyboards.Keyboard.EDGE_RIGHT;
import static wtf.uhoh.newsoftkeyboard.app.testing.ViewTestUtils.getKeyCenterPoint;
...
public class KeyboardViewTest extends KeyboardViewWithMiniKeyboardTest {

  private KeyboardView mViewUnderTest;
  private boolean mThemeWasSet;

  @Override
  protected KeyboardViewBase createViewToTest(Context context) {
    return new KeyboardView(context, null) {
  ...
  @Override
  protected void setCreatedKeyboardView(@NonNull KeyboardViewBase view) {
    super.setCreatedKeyboardView(view);
    mViewUnderTest = (KeyboardView) view;
  }

  @Test
  public void testKeyClickHappyPath() {
    KeyboardKey key = findKey('a');
  ...
  public void testKeyRepeatClickHappyPath() {
    KeyboardKey key = findKey(KeyCodes.DELETE);
  ...
  public void testKeyClickDomain() {
    ...
    KeyboardKey key = findKey(KeyCodes.DOMAIN);
    ...
    KeyboardViewBase miniKeyboard = mViewUnderTest.getMiniKeyboard();
  ...
  public void testMinimumPadding() {
    ...
    NskApplicationBase.getKeyboardThemeFactory(mViewUnderTest.getContext())
        .getEnabledAddOns()
        .forEach(
            keyboardTheme -> {
  ...
  public void testTouchIsDisabledOnGestureUntilAllPointersAreUp() {
    ...
    KeyboardKey key1 = (KeyboardKey) mEnglishKeyboard.getKeys().get(keyAIndex);
    KeyboardKey key2 = (KeyboardKey) mEnglishKeyboard.getKeys().get(keyJIndex);
  ...
  public void testSlideToNextKeyHappyPath() {
    KeyboardKey key1 = (KeyboardKey) mEnglishKeyboard.getKeys().get(14);
    KeyboardKey key2 = (KeyboardKey) mEnglishKeyboard.getKeys().get(15);
  ...
  public void testSwipeSpaceUpToUtilitiesKeyboardWithGestureTyping() {
    ...
    final KeyboardKey key = findKey('a');
    ViewTestUtils.navigateFromTo(mViewUnderTest, key, key, 30, true, true);
  ...
  public void testQuickTextPopupHappyPath() {
    KeyboardKey quickTextPopupKey = findKey(KeyCodes.QUICK_TEXT);
  ...
  public void testLongPressEnter() throws Exception {
    KeyboardKey enterKey = findKey(KeyCodes.ENTER);
  ...
  public void testWithLongPressDeleteKeyOutput() {
    final KeyboardKey key = findKey(KeyCodes.DELETE);
  ...
  public void testSwipeDownWhenGestureTypingIsDisabled() {
    ...
    final KeyboardKey topKey = requireFindKey('t');
    final KeyboardKey bottomKey = requireFindKey('c');
