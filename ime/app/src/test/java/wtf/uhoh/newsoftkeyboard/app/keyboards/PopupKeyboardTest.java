...
import wtf.uhoh.newsoftkeyboard.testing.MyShadowPaint;
...
public class PopupKeyboardTest {
  ...
  @NonNull
  private PopupKeyboard createPopupKeyboard(
  ...
  public void testKeyboardResourceConstructor() throws Exception {
    PopupKeyboard keyboard = createPopupKeyboard(R.xml.quick_text_unicode_emoticons, null, null);
  ...
  public void testKeyboardResourceConstructorReadsTags() throws Exception {
    ...
    Assert.assertArrayEquals(
        "face,grin".split(","), ((KeyboardKey) keyboard.getKeys().get(0)).getKeyTags().toArray());
    Assert.assertArrayEquals(
        "eye,face,grin,smile".split(","),
        ((KeyboardKey) keyboard.getKeys().get(1)).getKeyTags().toArray());
  }
  ...
  public void testKeyboardResourceConstructorReadsTagsWithExtension() throws Exception {
    ...
    Assert.assertArrayEquals(
        ...
        ((KeyboardKey) keyboard.getKeys().get(0)).getKeyTags().toArray());
  }

  private void assertKeyValues(PopupKeyboard keyboard, int primaryCode, int y) {
    assertKeyValues(keyboard, primaryCode, y, -1);
  }

  private void assertKeyValues(PopupKeyboard keyboard, int primaryCode, int y, int x) {
  ...
  public void testEmptyCodes() {
    PopupKeyboard keyboard =
        createPopupKeyboard(R.xml.keyboard_with_keys_with_no_codes, null, null);
  ...
  public void testKeyboardSwitchesSkinTone() throws Exception {
    PopupKeyboard keyboardWithGeneric =
        createPopupKeyboard(R.xml.quick_text_unicode_people, null, null);
    Assert.assertEquals("\uD83D\uDC85", keyboardWithGeneric.getKeys().get(1).text);
    Assert.assertEquals("\uD83D\uDC85", keyboardWithGeneric.getKeys().get(1).label);

    PopupKeyboard keyboardWithSkinTone =
        createPopupKeyboard(
  ...
  public void testHidesKeysWithNoGlyph() throws Exception {
    PopupKeyboard keyboard = createPopupKeyboard(R.xml.quick_text_unicode_people, null, null);

    MyShadowPaint.addStringWithoutGlyph(keyboard.getKeys().get(2).text.toString());

    keyboard = createPopupKeyboard(R.xml.quick_text_unicode_people, null, null);
  ...
  public void testDoesNotHideKeysWithJustText() throws Exception {
    ...
    PopupKeyboard keyboard = createPopupKeyboard(R.xml.popup_kaomoji, null, null);
