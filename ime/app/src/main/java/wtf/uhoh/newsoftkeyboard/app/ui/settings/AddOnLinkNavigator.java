...
final class AddOnLinkNavigator {
    ...
    try {
      ...
      if ("settings".equalsIgnoreCase(scheme)) {
        openAppSettings(host);
          ...
          || "nsk-settings".equalsIgnoreCase(scheme)) {
        openAddOnSettingsDestination(host, linkUri.getHost());
      } else {
    ...
    try {
      ...
      if ("settings".equalsIgnoreCase(scheme)) {
        ...
        return;
          ...
          || "nsk-settings".equalsIgnoreCase(scheme)) {
        openAddOnSettingsDestination(host, potentialUri.getHost());
        return;
      }
  ...
  private static void openAddOnSettingsDestination(@NonNull Fragment host, String destination) {
    if (destination == null) {
      Toast.makeText(
              host.requireContext(), R.string.prefs_providers_operation_failed, Toast.LENGTH_SHORT)
          .show();
