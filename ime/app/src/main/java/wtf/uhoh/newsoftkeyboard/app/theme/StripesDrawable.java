...
final class StripesDrawable extends Drawable {
  ...
  void setDirection(int direction) {
    final int normalized =
        direction == KeyboardWallpaperLayer.DIRECTION_HORIZONTAL
                || direction == KeyboardWallpaperLayer.DIRECTION_HORIZONTAL_REVERSE
  ...
  private void updateShaderMatrix() {
    ...
    if (direction == KeyboardWallpaperLayer.DIRECTION_HORIZONTAL) {
      shaderMatrix.postRotate(90f);
    }
  ...
  private static Bitmap getStripesBitmap() {
    ...
    synchronized (StripesDrawable.class) {
      if (cachedStripesBitmap != null && !cachedStripesBitmap.isRecycled())
        return cachedStripesBitmap;
