...
public abstract class ImeInlineSuggestions extends ImeSuggestionsController {
  ...
  private static final androidx.arch.core.util.Function<List<InlineSuggestion>, Void>
      NOOP_SHOW_SUGGESTIONS = suggestions -> null;
  private static final Runnable NOOP_ACTION = () -> {};
  private final InlineSuggestionsCallbacks mInlineSuggestionsCallbacks =
      new InlineSuggestionsCallbacks();

  private InlineSuggestionsAction mInlineSuggestionAction =
      new InlineSuggestionsAction(NOOP_SHOW_SUGGESTIONS, NOOP_ACTION);
  @Nullable private AutofillStripAction mAutofillStripAction;
  private boolean mAutofillRequestIssuedForCurrentEditor;

  @Override
  public void onCreate() {
    super.onCreate();
    mInlineSuggestionsCallbacks.attach(this);
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
      mInlineSuggestionAction =
          new InlineSuggestionsAction(
              mInlineSuggestionsCallbacks::showSuggestions,
              mInlineSuggestionsCallbacks::removeActionStrip);
    } else {
      mInlineSuggestionAction =
          new InlineSuggestionsAction(
              NOOP_SHOW_SUGGESTIONS, mInlineSuggestionsCallbacks::removeActionStrip);
    }
    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
      mAutofillStripAction =
          new AutofillStripAction(mInlineSuggestionsCallbacks::onAutofillStripActionPressed);
    } else {
  ...
  private static final class InlineSuggestionsCallbacks {
    private ImeInlineSuggestions mHost;

    void attach(@NonNull ImeInlineSuggestions host) {
      mHost = host;
    }

    @RequiresApi(Build.VERSION_CODES.R)
    private Void showSuggestions(@NonNull List<InlineSuggestion> inlineSuggestions) {
      return mHost.showSuggestions(inlineSuggestions);
    }

    private void removeActionStrip() {
      mHost.removeActionStrip();
    }

    @RequiresApi(Build.VERSION_CODES.O)
    private void onAutofillStripActionPressed() {
      mHost.onAutofillStripActionPressed();
    }
  }
}
