...
public class PopupKeyboard extends KeyboardDefinition {
  ...
  public PopupKeyboard(
    ...
    super(keyboardAddOn, hostAppContext, xmlLayoutResId, KEYBOARD_ROW_MODE_NORMAL);
  ...
  public PopupKeyboard(
    ...
    super(keyboardAddOn, hostAppContext, hostAppContext, getPopupLayout(popupCharacters));
  ...
  private static int getPopupLayout(CharSequence popupCharacters) {
    return switch (getPopupRowsCount(popupCharacters)) {
      ...
      default -> throw new RuntimeException("PopupKeyboard supports 1, 2, and 3 rows only!");
    };
  }
  ...
  private static Key findKeyWithSkinToneAndGender(
    ...
    final Predicate<KeyboardKey> checker;
    if (skinTone != null) {
  ...
  @Nullable
  private static KeyboardKey findKeyWithPredicate(List<Key> keys, Predicate<KeyboardKey> checker) {
    for (Key key : keys) {
      if (!(key instanceof KeyboardKey aKey)) {
        continue;
      }
  ...
  private void addPopupKeysToList(
    ...
    KeyboardKey baseKey = (KeyboardKey) keys.get(baseKeyIndex);
    Row row = baseKey.row;
  ...
  protected Key createKeyFromXml(
    ...
    final Key keyFromXml =
        super.createKeyFromXml(
            resourceMapping, hostAppContext, keyboardContext, parent, keyboardDimens, x, y, parser);
    if (!(keyFromXml instanceof KeyboardKey)) {
      return keyFromXml;
    }

    final KeyboardKey key = (KeyboardKey) keyFromXml;
