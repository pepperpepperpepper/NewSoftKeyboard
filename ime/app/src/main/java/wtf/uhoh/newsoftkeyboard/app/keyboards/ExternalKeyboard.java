...
import wtf.uhoh.newsoftkeyboard.app.keyboards.physical.HardKeyboardAction;
...
public class ExternalKeyboard extends KeyboardDefinition implements HardKeyboardTranslator {
  ...
  public ExternalKeyboard(
    ...
    this(
        ...
        sentenceSeparators,
        mode,
        NskApplicationBase.getKeyboardExtensionFactory(hostAppContext).getEnabledAddOn());
  }
  ...
  public ExternalKeyboard(
    ...
    super(
        keyboardAddOn,
        hostAppContext,
        getKeyboardId(hostAppContext, xmlLayoutResId, xmlLandscapeResId),
        mode);
    mName = name;
    mIconId = iconResId;
    ...
    if (key.mCodes.length > 0) {
      switch (key.getPrimaryCode()) {
        case 'a':
          ...
          key.popupResId = wtf.uhoh.newsoftkeyboard.R.xml.popup_one_row;
          break;
        case 'b':
