...
public abstract class MyMetaKeyKeyListener {
  ...
  /**
   * Call this method after you handle a keypress so that the meta state will be reset to unshifted
   * (if it is not still down) or primed to be reset to unshifted (once it is released). Takes the
  ...
  private static long press(long state, int what, long mask) {
    if ((state & (((long) what) << PRESSED_SHIFT)) != 0)
      ...
      state =
          (state | what | (((long) what) << PRESSED_SHIFT)) & ~(((long) what) << RELEASED_SHIFT);
    return state;
  }
  ...
  private static long release(long state, int what, long mask) {
    if ((state & (((long) what) << USED_SHIFT)) != 0) state = state & ~mask;
      ...
      state =
          (state | what | (((long) what) << RELEASED_SHIFT)) & ~(((long) what) << PRESSED_SHIFT);
    return state;
  }
}
