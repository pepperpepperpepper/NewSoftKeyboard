...
public class StickyModifiersInstrumentedTest {
  ...
  private void configureMikeRozoffAsDefault(Context appContext) {
    ...
    wtf.uhoh.newsoftkeyboard.app.keyboards.KeyboardFactory factory =
    ...
    for (wtf.uhoh.newsoftkeyboard.app.keyboards.KeyboardAddOnAndBuilder addOn :
        factory.getAllAddOns()) {
  ...
  public void modifierKeysShowBlueWhileActive() throws Exception {
    ...
    focusTestEditor();
    waitForKeyboardSurface();
    ImeServiceBase imeInstance = waitForImeInstance(KEYBOARD_READY_TIMEOUT_MS);
    Assume.assumeTrue("IME not initialized", imeInstance != null);
    ...
    InstrumentationRegistry.getInstrumentation()
        .runOnMainSync(
            () -> {
              ...
              if (ime == null) {
                android.util.Log.w("FunctionColorTest", "IME instance is null");
  ...
  @Nullable
  private ImeServiceBase waitForImeInstance(long timeoutMs) {
    long deadline = SystemClock.uptimeMillis() + timeoutMs;
    ImeServiceBase ime = ImeServiceBase.getInstance();
    while (ime == null && SystemClock.uptimeMillis() < deadline) {
      SystemClock.sleep(POLL_INTERVAL_MS);
      ime = ImeServiceBase.getInstance();
    }
    return ime;
  }
  ...
  private PointF findKeyCenterFromService(int primaryCode) {
    ImeServiceBase service = ImeServiceBase.getInstance();
    if (service == null) {
      android.util.Log.d(
          "StickyModifiersTest", "IME instance null when locating code " + primaryCode);
      return null;
    }
