package wtf.uhoh.newsoftkeyboard.gesturetyping;
...
public class GestureTypingDetector {
  private static final String TAG = "NSKGestureTypingDetector";
  ...
  public interface GestureKey {
    int getCodesCount();

    int getCodeAtIndex(int index);
  ...
  @NonNull private final Iterable<GestureKey> mKeys;

  @NonNull private SparseArray<GestureKey> mKeysByCharacter = new SparseArray<>();
  ...
  public GestureTypingDetector(
      ...
      @NonNull Iterable<GestureKey> keys) {
  ...
  private static Single<LoadingState> generateCornersInBackground(
      ...
      Iterable<GestureKey> keys,
      SparseArray<GestureKey> keysByCharacter,
                ...
                Observable.<LoadingState>create(
                    e -> {
                      ...
                      if (data.mKeysByCharacter.size() == 0) {
                        for (GestureKey key : data.mKeys) {
                          for (int i = 0; i < key.getCodesCount(); ++i) {
                            char c = Character.toLowerCase((char) key.getCodeAtIndex(i));
  ...
  private static int[] generatePath(
      char[] word, SparseArray<GestureKey> keysByCharacter, WorkspaceData workspaceData) {
    workspaceData.reset();
    ...
    for (char c : word) {
      ...
      GestureKey keyHit = keysByCharacter.get(c);
  ...
  public ArrayList<String> getCandidates() {
    ...
    GestureKey startKey = null;
    for (GestureKey k : mKeys) {
    ...
    for (int dictIndex = 0; dictIndex < mWords.size(); dictIndex++) {
      ...
      for (int i = 0; i < words.length; i++) {
        ...
        final GestureKey wordStartKey = mKeysByCharacter.get(Dictionary.toLowerCase(words[i][0]));
  ...
  private static class CornersGenerationData {
    ...
    private final Iterable<GestureKey> mKeys;
    private final SparseArray<GestureKey> mKeysByCharacter;
