...
/**
 * Generic notification system for AnySoftKeyboard addons.
 * Follows the same pattern as TesterNotification but allows addons to register their
 * own notifications when they are first loaded.
 */
public class AddonNotificationSystem {
  ...
  /**
   * Shows a notification for an addon if this addon hasn't shown a notification before.
   * This method can be called directly from addons.
   * 
  ...
  public static boolean showIfNeeded(
    ...
    if (!sp.getBoolean(key, false)) {
      ...
      PendingIntent contentIntent = PendingIntent.getActivity(
          sApplicationContext, 0, intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK),
          CompatUtils.appendImmutableFlag(0));

      var builder = sNotificationDriver
          .buildNotification(
          ...
          .setColor(
              ContextCompat.getColor(sApplicationContext, R.color.notification_background_debug_version))
          .setAutoCancel(true);
  ...
  /**
    * Shows a notification for an addon if this addon hasn't shown a notification before.
    * Overloaded method that accepts string parameters instead of resource IDs.
    * 
    * @param packageName The package name of the addon (used as unique identifier)
    * @param intent The intent to open when notification is clicked
