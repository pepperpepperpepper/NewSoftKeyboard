...
import com.google.android.voiceime.backends.GroqSpeechBackend;
...
public class GroqSpeechBackendTest {

  private Context mContext;
  private InMemorySharedPreferences mPrefs;
  private GroqSpeechBackend mBackend;
  ...
  public void setUp() {
    ...
    Mockito.when(mContext.getString(R.string.settings_key_groq_api_key))
        .thenReturn("settings_key_groq_api_key");
    mPrefs = new InMemorySharedPreferences();
    mBackend = new GroqSpeechBackend();
  }
  ...
  public void testIsNotSelectedByDefaultWhenUnset() {
    // Default OFF: no audio is sent to Groq until the user explicitly opts in.
    Assert.assertFalse(mBackend.isSelected(mContext, mPrefs));
  }

  @Test
  public void testIsSelectedMatchesBackendValue() {
    mPrefs.edit().putString("settings_key_speech_to_text_backend", GroqSpeechBackend.ID).apply();
  ...
  public void testIsConfiguredRequiresApiKey() {
    mPrefs
        .edit()
        .putString("settings_key_speech_to_text_backend", GroqSpeechBackend.ID)
        .putString("settings_key_groq_api_key", "")
        .apply();

    Assert.assertFalse(mBackend.isConfigured(mContext, mPrefs));

    mPrefs.edit().putString("settings_key_groq_api_key", "gsk-123").apply();
